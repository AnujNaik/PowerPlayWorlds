...
public class MotorPIDTuner extends LinearOpMode {
    public static double COUNTS = 1300; // counts
    ...
    private static MotionProfile generateProfile(boolean extending) {
        MotionState start = new MotionState(extending ? 0 : ((COUNTS / 384.5) * (30 / 25.4)), 0, 0, 0);
        MotionState goal = new MotionState(extending ? ((COUNTS / 384.5) * (30 / 25.4)) : 0, 0, 0, 0);
    ...
    public void runOpMode() {
        ...
        while (!isStopRequested()) {
            ...
            double targetPower = pidfController.calculate(dropperBack.getCurrentPosition(), COUNTS);

            dropperBack.setPower(targetPower);
            dropperFront.setPower(targetPower);

            double currentVelo = dropperBack.getVelocity();
